...
theta = np.logspace(np.log10(theta_min), np.log10(theta_max), vec_len)
...
def get_IA_zlims(year):
    ...
    rpmin = setup.arcmin_to_rp(theta[0], zleff, halo.cosmo)
    rpmax = setup.arcmin_to_rp(theta[-1], zleff, halo.cosmo)

    r_p = np.logspace(np.log10(rpmin), np.log10(rpmax), vec_len)
...
def get_xi_gg(year, maxPi, minPi):
    ...
    # set some arbitrary projected separations in theta range, r_p, for final xi_ls, requires rp < r
    r_p = np.logspace(np.log10(rpmin), np.log10(rpmax), vec_len)
    
    # define max and min r based on combination of rp and Pi
    rmax = np.sqrt(rpmax**2 + (maxPi+1)**2)
    rmin = np.sqrt(rpmin**2 - minPi**2)

    # set some arbitrary 3D separations r to compute xi_gg
    r = np.logspace(np.log10(rmin), np.log10(rmax), vec_len)

    # wavenumbers and scale factors for all-space power spectrum
    k_arr = np.geomspace(1E-4, 5E2, 3*vec_len)
...
def get_xi_ls(year):
    ...
    return xi_ls, r_p, Pi, z_Pi

def get_boosts(year):
    
    xi_ls, r_p, Pi, z_Pi = get_xi_ls(year)
    ...
    np.savez('Boost_data_year%d'%year, B_min1 = igl_zl, B = igl_zl + 1., rp = r_p, theta = theta)
